...
A target-free refund is an opt-in v0.4.8 financial record and therefore requires a string `installation_id`. It is resolved server-side by exactly one canonical settled purchase sharing `(tenant_id, app_id, installation_id, original_transaction_id, currency)`. For a purchase, the canonical original transaction is `original_transaction_id` when present and otherwise `transaction_id`. Only installation-anchored, base-accepted purchases that survive record-ID collision checks and logical/business canonicalization are candidates; a rejected purchase can never make the set ambiguous or throw the batch. Target-free candidates additionally require both `purchase.received_at <= refund.received_at` and `purchase.occurred_at <= refund.occurred_at`; equal timestamps are eligible. This point-in-time boundary makes the persisted target append-only: a later purchase cannot retroactively make an accepted refund ambiguous.

An installation-anchored refund with an explicit `correction_target_record_id` uses the same v0.4.8 financial checks and additionally requires the resolved purchase record to equal that assertion. An unanchored refund is accepted only when it names an explicit target; it retains the v0.4.0 correction behavior: the named record must exist in the authenticated tenant/app scope, but financial status, receipt order, occurrence order, installation, original transaction, currency, cap, and business-transaction rules do not reclassify it. Such a legacy correction remains event-ID-idempotent and does not project to `refund_facts` or purchase-net metrics. Fixture 16 records this out-of-order legacy behavior and does not weaken target-free exactly-one resolution.

Installation-anchored settled refunds for one target are ordered append-only by `(received_at, record_id, delivery_id)` and folded against the purchase's exact same-currency amount after aligning decimal scales. A refund is admitted only after the base, target, and cap checks all pass. Only then does it reserve its business transaction identity and increase the fold; an invalid earlier attempt therefore cannot block a later valid payload with the same transaction ID. Zero candidates, multiple target-free candidates, an explicit target mismatch, a pre-purchase refund, or an over-refund all fail closed as `refund_target_invalid` with discarded payload and non-identifying rejection metadata.
...
Installation-anchored purchase and refund events also have the business identity `(server tenant_id, server app_id, event_name, transaction_id)`. Different logical event IDs carrying the same business identity use deterministic first-accepted-wins ordering by `(received_at, record_id, delivery_id)` plus the canonical tie-breakers. A refund reserves that identity only after its base, target, and cap checks pass. RFC-8785-equivalent later payloads are `duplicate_delivery`; a later payload that differs in amount, status, original transaction, or any other field fails closed as `event_id_conflict` without retroactively invalidating the already accepted financial fact. Unanchored legacy commerce remains event-ID-idempotent. Input array order therefore cannot choose or erase an anchored fact.
...
For `duplicate_delivery` and `event_id_conflict`, `canonical_record_id` is the `record_id` of the first accepted delivery for the applicable logical or installation-anchored business idempotency key. It identifies the accepted canonical record without replacing later delivery evidence. A `record_id_collision` has no canonical record and therefore must not emit `canonical_record_id`.
...
| `adapter:<network>` | Deployment-private media API adapter, including bounded cost collection. |
| `postback:<kind>` | S2S or platform postback receiver, including later SKAN or AdAttributionKit work. |

The provider, network, and postback-kind suffixes are deployment-private mappings. Registry patterns validate syntax only; they do not authorize a source. A runtime implementation must enforce a tenant-scoped private allowlist before contract evaluation. Public fixtures and their golden outputs use only synthetic suffixes.
...
The canonical provider dimension field names are `provider_network`, `provider_site_ref`, and `provider_country`. The provider prefix keeps imported dimensions distinct from first-party dimensions.
...
`skan_postback` and `adattributionkit_postback` are protected, aggregate event envelopes. Their authenticated producer uses a deployment-private `postback:<kind>` mapping; public fixtures use only `postback:synthetic-*` values. Signature material and `signature_verified` are separate fields. Contract v0.4 supports SKAdNetwork major versions 3 and 4, including later minor versions such as 4.1; legacy v1/v2 and future major versions are outside this contract. Version 3 uses `campaign_id` and does not accept v4-only source, sequence, or coarse-value fields. Version 4 requires `postback_sequence_index`, forbids `campaign_id`, and permits at most one of fine and coarse conversion value; fine conversion value is accepted only in sequence 0. AdAttributionKit conversion types are `download`, `redownload`, and `re-engagement`. A `re-engagement` postback is accepted only with `ad_interaction_type=click` and `did_win=true`; it remains aggregate evidence and is never converted into a device-level deep-link or installation subject. These constraints follow Apple's postback parameter and receiving specifications checked on 2026-08-30: <https://developer.apple.com/documentation/adattributionkit/identifying-the-parameters-in-a-postback> and <https://developer.apple.com/documentation/adattributionkit/receiving-ad-attributions-and-postbacks>. The optional `signing_key_environment=production | development` records which verified Apple public-key set selected the JWS `kid`; development keys MUST be rejected unless deployment policy explicitly enables them. The reference evaluator emits:
...
The closed `purchase_net_revenue` numerator is used by six built-in definitions. D0, D1, D3, and D7 retain metric-definition and rule-bundle version `0.4.8`, rule bundle `metric-purchase-net`, and fixed hash `8888888888888888888888888888888888888888888888888888888888888888`. D30 and D90 are additive version-`0.4.9` definitions with the same bundle ID and fixed hash `9999999999999999999999999999999999999999999999999999999999999999`. All six use UTC elapsed windows, USD scale 6, and `revenue_sum`. An installation-anchored settled purchase contributes positive converted money when its own `occurred_at` falls in the install-anchored half-open window `[install.occurred_at, install.occurred_at + (day + 1 days))`. A valid installation-anchored settled refund subtracts converted money when the refund's own `occurred_at` falls in that window; it never restates the target purchase's earlier window. Pending and reversed anchored purchases and refunds do not contribute, the per-target cap prevents a valid cumulative net from subtracting more than its matched purchase, and unanchored legacy commerce is excluded.

Version `0.4.9` adds the closed `total_net_revenue` numerator for D30 and D90 only. It is the sum of existing installation-level advertising revenue and `purchase_net_revenue`, after each source money event is independently converted and half-even rounded. The six total-net definitions are `cohort_total_net_revenue_d30_usd`, `cohort_total_net_revenue_d90_usd`, `d30_total_net_roas`, `d90_total_net_roas`, `cohort_total_net_ltv_d30_usd`, and `cohort_total_net_ltv_d90_usd`; all are bound to `metric-total-net` / `0.4.9` / the fixed all-`9` hash. ROAS uses the existing acquisition-day current cost snapshot and LTV uses the existing distinct installation cohort size. The existing `revenue` numerator remains advertising revenue only, so every pre-0.4.9 ad-revenue, ROAS, and LTV definition remains byte-for-byte unchanged. A run may claim `data_freshness=complete` only when its fixed watermark is at or after the exclusive horizon end for every included installation; earlier operational runs must use a non-complete freshness state and remain subject to superseding recomputation.

This financial status is provider-neutral, client-reported contract evidence. It does not prove App Store, Google Play, or private-provider verification. Historical `purchase_facts.financial_status IS NULL` rows are not implicitly treated as settled and are not automatically backfilled; using them requires an explicit replay from protected source payload under operator control. Live or private-provider replay is outside this public synthetic contract slice.
...
Apple aggregate postback metrics retain the three closed v0.3.3 definitions `skan_attributed_installs`, `skan_conversion_value_distribution`, and `aak_attributed_installs`, and v0.4.10 adds `aak_attributed_reengagements`. Despite the historical `*_installs` aliases, these values count qualified, accepted, deduplicated postback events; they do not claim a unique-device or unique-install denominator that Apple does not expose. `aak_attributed_installs` accepts only `download` or `redownload`, while `aak_attributed_reengagements` accepts only `re-engagement`, so re-engagement never inflates the install series. A postback qualifies only when its aggregate attribution is `non_organic`, which requires a verified signature, `did_win=true`, a source identifier, and a fine or coarse conversion value. The metric day is derived from the protected server `received_at` in UTC because the aggregate postback does not carry an occurrence time. `attribution_status` grouping is forbidden for these already-qualified aggregate series.
...
2. `provider_click_ref`, when present, maps to the protected, tenant/app-scoped, one-to-one `provider_click_id` matching-key type. The payload field keeps the opaque-reference name while the matching-key registry keeps its canonical type name.
...
5. validates all 741 golden output artifacts;
6. runs named assertions for all 57 scenarios and 27 acceptance criteria (AC01-AC27);
...
### v0.4.10 patch release

- AdAttributionKit postbacks add the current `re-engagement` conversion type, constrained to click-through winning postbacks and preserved as aggregate evidence.
- The new `aak_attributed_reengagements` definition reports qualified re-engagement postbacks separately; `aak_attributed_installs` remains limited to `download` and `redownload`.
- Fixture 57 supplies all 13 reviewed golden classes. TypeScript and independent Python evaluators produce the same RFC 8785 JCS digest, and all earlier fixture inputs and goldens remain byte-identical.

### v0.4.9 patch release

- This patch adds D30/D90 purchase-net revenue and the independent `total_net_revenue` numerator. Total-net revenue is advertising revenue plus settled installation-anchored purchases minus admitted settled refunds; existing ad-only definitions are unchanged.
- Six fixed-provenance total-net definitions provide D30/D90 revenue, ROAS, and cohort LTV. All use UTC elapsed half-open windows, per-event half-even FX, the existing cost snapshot, and the existing cohort denominator.
- Fixture 56 supplies all 13 reviewed golden classes and proves the D31 and D91 exclusive boundaries. Existing fixture inputs and goldens remain byte-identical.

### v0.4.8 patch release

- Purchase and refund event payloads add optional `installation_id`; a refund requires either that installation anchor or an explicit `correction_target_record_id`. Delivery and rejection vocabularies add `refund_target_invalid`, and metric definitions add the closed `purchase_net_revenue` numerator.
- A string `installation_id` opts commerce into v0.4.8 strict targeting, refund facts, caps, business transaction canonicalization, and purchase-net metrics. Target-free server resolution requires exactly one base-accepted, canonical, same-tenant/app/installation/original-transaction/currency purchase known at refund receipt. An anchored explicit target is an assertion over that strict resolution. Missing, ambiguous, explicit-mismatch, pre-purchase, and cumulatively over-refunded anchored targets fail closed. Refund caps use append-only receipt order, and only fully admissible refunds reserve a business transaction identity.
- Unanchored purchases and explicit-target refunds retain v0.4.0 event-ID-only behavior. The existing purchase projection remains compatible. The explicit refund target must exist in the same tenant/app scope, but no new financial cross-check reclassifies that legacy correction; it emits no refund fact, and neither unanchored event contributes to purchase-net metrics. Fixture 16 therefore remains an out-of-order compatibility case.
- Fixture 55 supplies all 13 reviewed golden classes for the four fixed-provenance version-0.4.8 purchase-net definitions and independently preserves the existing ad-revenue result. Fixture 16 and all 13 of its reviewed goldens remain byte-identical as the explicit legacy out-of-order receipt case. Schema `$id` values and event-version constants remain on the v0.4 / `0.4.0` line.

### v0.4.7 patch release

- This patch adds the independent `deep_link_open` event, engagement-level deep-link attribution vocabulary, optional deferred-destination evidence on clicks and installs, and daily deep-link event-count definitions.
...
- This patch adds `ctit_clock_anomaly` as a non-fraud `clear` / `allow` diagnostic and admits a registered fraud-bundle revision in fixture server context.
...
- This patch adds the `payload_schema_invalid` rejection reason and a closed fixture-only pre-ingestion decision envelope. Existing event payloads and artifact meanings are unchanged.
...
- This patch reserves optional, server-assigned `integrity_verdict` evidence on raw records and fixture ingress envelopes. The closed evidence records a platform (`play_integrity` or `app_attest`), a neutral verdict (`verified`, `failed`, or `unavailable`), evaluation time, policy version, and a protected opaque evidence reference when evidence exists.
...
- No typed install-schema change is required. The bundled iOS conversion schema version and SHA-256 digest remain deployment-private evidence in the existing `install.extensions` surface.
...
- This patch adds Apple aggregate postback metric semantics without changing any existing event or metric definition. The `event_count` selector admits `skan_postback` and `adattributionkit_postback`; new aggregate definitions use UTC `received_at` calendar days and reject `attribution_status` grouping.
...
- The optional AdServices outcomes `not_attributed` and `lookup_unavailable` each require `attribution=false` and retain a distinct neutral attribution reason.
- `referrer_status=not_applicable`, `install_origin=ios_first_launch`, and `platform_referrer_not_available` prevent an iOS first launch from pretending that the Play Install Referrer path exists.
- The optional AdAttributionKit signing-key environment records which verified key set was selected. An advertiser copy cannot support a win-rate denominator because it contains winners only.
- Later minor versions within supported SKAdNetwork majors 3 and 4 are accepted; v1, v2, and unknown future majors remain rejected.
...
- This patch adds the optional `metric_date` grouping dimension to metric definitions, metric runs, and fixture evaluations. Existing groupings and every v0.3.0 golden retain their meaning.
...
- Aggregate attribution-method display requires no new artifact field: an aggregate is identified by its rule-bundle ID/version/hash and broken down by `attribution_status`. A single attribution method label would be false for an aggregate that may mix decision methods.
...
- The v0.2.1 patch adds optional metric-run `value_state` and `undefined_reason` fields. Omitted `value_state` retains the v0.2.0 present-value meaning, so existing metric-run goldens are unchanged.
- The same patch adds `provider_modeled_conversion` to the difference-reason registry and reconciliation schema. Only that new v0.2.1 reason uses `difference_reason_version=0.2.1`; older reasons remain `0.2.0`. The v0.3 migration advances every reconciliation reason to `0.3.0`.
...
- The imported-attribution extension adds a separate imported method, closed import context, automatic reconciliation, explicit producer forms, and `timestamp_stale`; none of these changes name or rate an incumbent provider.
- Reporting extensions add typed click/install dimensions, advertiser-side `ad_view`, scoped ad revenue, append-only cost records, data-driven metric definitions, per-event half-even conversion, and deterministic ROAS, retention, and cohort metrics.
- Platform extensions add aggregate SKAdNetwork and AdAttributionKit envelopes, a closed minimal Meta Install Referrer envelope, Apple AdServices evidence, compatibility rows, and versioned platform attribution reasons. Provider fields that could not be verified from primary documentation are deliberately absent.
- Privacy extensions add the closed processing-purpose catalog, authenticated tenant-admin and same-installation on-device request provenance, and optional app audience. The `general` audience remains the default, while child-directed payloads structurally reject reserved advertising identifiers.
