...
Click-to-install time (CTIT) is `install_begin_at_server - redirector_click_at`, using only server-authoritative timestamps. `redirector_click_at` is the canonical v0.4 field corresponding to the historical work-order term `referrer_click_at_server`. The fixture server context carries a closed `click_injection_policy`; its default threshold classifies a nonnegative CTIT strictly below 10 seconds as `click_injection_suspected`. Exactly 10 seconds is outside that category. This public rule demonstrates deterministic contract behavior only. Production thresholds, features, and response policy remain deployment-private, versioned controls.
