...
describe("dashboard session primitives", () => {
  it("C01 selects the secure prefix only for HTTPS and emits the fixed attributes", () => {
    ...
    assert.equal(dashboardCookieName("https://measure.example.invalid"), "__Host-openmasu_dashboard");
    assert.equal(dashboardCookieName("http://localhost:8080"), "openmasu_dashboard");
    assert.equal(
      dashboardSessionCookie(token, "https://measure.example.invalid", 43200),
      `__Host-openmasu_dashboard=${token}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=43200`,
    );
  ...
  it("C02 reads only a full 32-byte base64url token from the configured cookie", () => {
    ...
    assert.equal(readDashboardToken(`other=x; openmasu_dashboard=${token}`, "http://localhost:8080"), token);
    assert.equal(readDashboardToken("openmasu_dashboard=x", "http://localhost:8080"), undefined);
    assert.equal(readDashboardToken(`__Host-openmasu_dashboard=${token}`, "http://localhost:8080"), undefined);
  });
  ...
  it("C07 refuses plain HTTP outside loopback and accepts the supported origins", () => {
    assert.throws(
      ...
      /OPENMASU_DASHBOARD_INSECURE_ORIGIN.*proxy/,
    );
