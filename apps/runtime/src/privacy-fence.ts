  ...
  try {
    ...
    return {
      ...
      release: async () => {
        ...
        released = true;
        try {
          await client.query("SELECT pg_advisory_unlock_shared(hashtextextended($1,0))", [lockKey]);
          client.release();
        } catch (error) {
    ...
    if (locked) {
      try { await client.query("SELECT pg_advisory_unlock_shared(hashtextextended($1,0))", [lockKey]); }
      catch (unlockError) { cleanupError ??= unlockError; }
    }
    if (cleanupError) client.release(asError(cleanupError));
    else client.release();
    if (cleanupError) throw new AggregateError([error, cleanupError], "privacy fence setup failed");
    throw error;
  }
}
