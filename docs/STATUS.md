...
Checked on 2026-08-24. “Implemented” below means source and synthetic automated
...
| M2 Android/Unity/redirector | Implemented | HMAC durable ingestion, deterministic referrer path, Kotlin SDK, Unity bridge, synthetic Google Play one-time-product, initial-subscription, and authenticated RTDN latest-renewal order-total verification, emulator CI, Android SBOM | Real Play Billing/Pub/Sub credentials, tokens, notifications, orders, missed-notification renewal backfill, broader lifecycle, Play/Meta/MAX campaigns, devices, refunds, backup transfer, and Unity export remain operator checks |
