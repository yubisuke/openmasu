...
| `fixtures/v0.2/` | `fixtures/v0.4/` | Reviewed inputs and golden outputs are a versioned set. |
| `spec/event-metric-contract-v0.2.md` | `spec/event-metric-contract-v0.4.md` | The normative specification follows the active minor line. |
...
The authoritative derivation details for fixtures 39-43 are also recorded in `fixtures/v0.4/README.md`. No fixture is generated by the validator.
...
| Custom event | Admits the exact reserved key `openmasu.conversion_value_updated` in addition to the existing public-key pattern | Additive lifecycle event used only when conversion-value logging is explicitly enabled; existing custom events retain their meaning. |
