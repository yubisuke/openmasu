...
- [ ] **Unity export.** Export from the UPM package on each supported Unity line. On Android, verify the merged manifest contains exactly one OpenMasu `autoVerify` filter per `ProjectSettings/OpenMasuAndroidSettings.json` host, no synthetic default host, matching `dev.openmasu.sdk.LINK_HOSTS` metadata, and unchanged operator-owned custom-scheme filters. Confirm runtime initialization accepts the same host list and rejects a mismatch. On iOS, verify entitlement injection without hand-editing.
