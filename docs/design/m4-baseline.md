...
4. **Whether Unity compiles Swift source plugins without a build post-processor**, whether a UPM package's `PrivacyInfo.xcprivacy` reaches the built project, and whether M7's generated Associated Domains entitlement survives export. These remain settled by real Unity builds (M4-V-7 and DL-V-7), with the `.xcframework` fallback already designed; see the [Deep-Link Design Baseline](deeplink-baseline.md) (2026-08-21).
