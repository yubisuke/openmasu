...
| One active commerce read-back cannot race another worker or completed deletion | Complete with a per-row database-clock lease, token-fenced retry, transactional Google refund and Apple cursor completion, expired-claim recovery, stale-completion rejection, accepted-purchase App Store linkage, terminal cursor cleanup, and Google/Apple deletion-race evidence; unmatched App Store installation linkage and provider-side exactly-once behavior remain unverified |
