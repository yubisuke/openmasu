...
| Google conversion delivery fencing and pacing | Complete for the Google Data Manager delivery queue with per-row database-clock claims, token-fenced completion, and one database-backed request slot per destination | Synthetic processors prove active-claim exclusion, expired-claim recovery, stale-completion rejection, stable transaction-ID reuse, distinct-row pacing, bounded `Retry-After` propagation, and no false attempt append; live quotas and provider-side exactly-once behavior remain operator boundaries |
