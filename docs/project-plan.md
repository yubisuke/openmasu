...
| Commerce read-back completion fencing | Complete for Google Play lifecycle/refund and App Store history queues with per-row database-clock claims and privacy-fenced completion | Concurrent, expired-claim recovery, stale-cursor rejection, accepted-purchase App Store linkage, cursor cleanup, and Google/Apple deletion-race tests prove local ownership and no derived-state resurrection; unmatched App Store installation linkage and provider-side exactly-once behavior remain operator boundaries |
