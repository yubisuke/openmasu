...
| Google Data Manager conversion delivery | Explicitly enabled, bounded service-account delivery of eligible verified conversions with per-row durable claims, fenced completion, a stable transaction ID, and destination-scoped database pacing | Synthetic authentication, request, retry, active-claim exclusion, distinct-row pacing, bounded `Retry-After`, expired-claim recovery, stale-completion rejection, and diagnostic tests | Provider access, acceptance, same-transaction-ID duplicate behavior, live quota allocation, diagnostics latency, production policy |
