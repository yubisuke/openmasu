...
| AdAttributionKit | Signed developer-copy receiver; install and re-engagement aggregate series; iOS and Unity conversion-type/tag helpers | Synthetic JWS fixture, receiver/replay tests, TypeScript/Python/PostgreSQL parity, macOS SDK and Unity bridge gates | Real copies, Info.plist propagation, Unity export, device conversion windows, caller-owned conversion-tag lifecycle |
...
| Google Data Manager conversion delivery | Explicitly enabled, bounded service-account delivery of eligible verified conversions with per-row durable claims, fenced completion, and a stable transaction ID | Synthetic authentication, request, retry, active-claim exclusion, expired-claim recovery, stale-completion rejection, and diagnostic tests | Provider access, acceptance, same-transaction-ID duplicate behavior, quota, diagnostics latency, production policy |
