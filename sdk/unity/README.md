...
The package in `com.openmmp.sdk` targets Unity 2022.3 LTS (best effort) and Unity 6 LTS. Android `minSdk` is 24.

M2 ships source and local build instructions only. It does not publish a Maven or UPM artifact. Build the Android modules in `sdk/android`, publish the resulting dependency to an operator-controlled local Maven repository, and replace the placeholder `dev.openmmp:core:0.1.0` coordinate in `Runtime/OpenMmp.androidlib/build.gradle`.
