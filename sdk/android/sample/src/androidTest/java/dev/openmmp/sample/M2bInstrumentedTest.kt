...
import dev.openmmp.sdk.InstallationCredential
import dev.openmmp.sdk.MetaReferrerEvidence
import dev.openmmp.sdk.MetaReferrerReader
...
class M2bInstrumentedTest {
  ...
  @Before fun cleanState() {
    ...
    context.getSharedPreferences("openmmp_private", Context.MODE_PRIVATE).edit().clear().commit()
    File(context.filesDir, "openmmp").deleteRecursively()
  }

  @Test fun emulatorSmokeDeliversOneInstallThroughTheSignedHttpPath() {
    LocalSdkApi().use { api ->
      val sdk = OpenMmpSdk.create(
        context,
        OpenMmpConfiguration(api.endpoint, "sdk-key:synthetic", "synthetic-sdk-secret"),
      ...
      assertEquals(setOf("openmmp"), context.filesDir.list()?.toSet().orEmpty())
      sdk.close()
    }
  }
  ...
  private fun marker() = File(context.filesDir, "openmmp/durability-ready")
  ...
  private fun assertQueueCount(expected: Int) {
    val database = OpenMmpQueueDatabase.open(context)
  ...
  private class LocalSdkApi : AutoCloseable {
    ...
    private val thread = Thread(::serve, "openmmp-local-api")
