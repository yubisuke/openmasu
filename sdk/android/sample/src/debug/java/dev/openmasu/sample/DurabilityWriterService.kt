...
import dev.openmasu.sdk.QueuedEvent
import java.io.File
...
class DurabilityWriterService : Service() {
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    ...
    Thread({ runProbe(action) }, "openmasu-durability-writer").start()
    return START_NOT_STICKY
  }

  private fun runProbe(action: String) {
    val database = OpenMasuQueueDatabase.open(this)
  ...
  private fun markerFile(): File = File(filesDir, "openmasu/durability-ready").apply { parentFile?.mkdirs() }

  companion object {
    const val ACTION_SEED = "dev.openmasu.sample.SEED_QUEUE"
    const val ACTION_REWRITE_DURING_KILL = "dev.openmasu.sample.REWRITE_QUEUE"
  }
}
